...
var canHit = true; //Можно нажимать кнопку 'Ещё' пока значение YourSum <= 21

window.onload = function () {//запуск функций:Построить колоду, перемешать колоду, начать игру
    buildDeck();
    shuffleDeck();
    startGame();
}
...
function startGame() {
    ...
    while (dealerSum < 17) {  //Если сумма карт диллера < 17, то выдать еще одну карту
        ...
        var card = deck.pop();
        cardImg.src = "Sourses/Pictures/cards/" + card + ".png";//выбор картинки карты в соотвествии с ее значение
    ...
    for (var i = 0; i < 2; i++) { //
        ...
        var card = deck.pop();
        cardImg.src = "Sourses/Pictures/cards/" + card + ".png";
...
function stay() { //Функция stay = она же хватит, она же вскрываемся P.S. образно говоря Hit= Еще, Stay = Вскрываемся просто так выглядит цивильней
    ...
    canHit = false;
    document.getElementById("hidden").src = "Sourses/Pictures/cards/" + hidden + ".png";
